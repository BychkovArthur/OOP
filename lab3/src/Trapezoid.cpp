...
void Trapezoid::calcHeight()
{
    ...
    height = std::sqrt(std::pow(sideLength1, 2) - std::pow(
        (std::pow(smallerBaseLength - biggerBaseLength, 2) + std::pow(sideLength1, 2) -
        std::pow(sideLength2, 2)) / (2 * (smallerBaseLength - biggerBaseLength)), 2
    ));
...
Point Trapezoid::getGeometricCenter() const
{
    ...
    geometricCenter.y += (height / 3.0) * ((2 * biggerBaseLength) + smallerBaseLength) / (biggerBaseLength + smallerBaseLength);
